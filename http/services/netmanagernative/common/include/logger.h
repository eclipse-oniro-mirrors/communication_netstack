...
#define LogError nmd::common::logger::error() << "[File:'" << __FILE__ << "',Line:'" << __LINE__ << "']: "

namespace nmd {
namespace common {

typedef enum log_level { INFO = 0, DEBUG, ERROR, WARN, FATAL } log_level;
...
template<typename T>
logger &operator<<(nmd::common::logger &log, T const &value)
{
    std::lock_guard<std::mutex> lo(log.mu_);
