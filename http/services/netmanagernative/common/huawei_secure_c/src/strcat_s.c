 ...
 *    The strcat_s function appends a copy of the string pointed to by strSrc (including the terminating null
 * character) to the end of the  string pointed to by strDest. The initial character of strSrc overwrites the
 * terminating null character of strDest. strcat_s will return EOVERLAP_AND_RESET if the source and destination
 * strings overlap.
 *
 ...
 *                         (strDest != NULL and strSrc is NULL and destMax != 0 and destMax <=
 * SECUREC_STRING_MAX_LEN) ERANGE              destMax is 0 and destMax > SECUREC_STRING_MAX_LEN ERANGE_AND_RESET
 * strDest have not enough space  and all other parameters are valid  and not overlap EOVERLAP_AND_RESET   dest
 * buffer and source buffer are overlapped and all  parameters are valid
 *
